...
if row is not None:
    print """Restarting existing crawl.
        Remove spider.db to start a fresh crawl."""
else:
    starturl = raw_input('Enter web url or enter: ')
    if (len(starturl) < 1 ):
        starturl = 'http://www.people.com.cn/'
    if (starturl.endswith('/')):
        starturl = starturl[:-1]
    web = starturl
    if (starturl.endswith('.htm') or starturl.endswith('.html')):
        pos = starturl.rfind('/') # rfind: 从右边开始找起
        web = starturl[:pos]
...
while True:
    ...
    try:
        ...
        print 'No nuretrieved HTML pages found'
        many = 0
        break
    ...
    try:
        ...
        # Normal Unless you encounter certificate problem
    ...
    for tag in tags:
        ...
        # Resolive relative references like href="/contace"
        up = urlparse(href)
        if (len(up.scheme) < 1):
